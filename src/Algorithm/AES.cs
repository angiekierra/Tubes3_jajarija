...
public class AES
{
    ...
    public static List<byte[]> SplitAndPadInput(string input)
    {
        List<byte[]> paddedBlocks = new List<byte[]>();

        // Pad the input to a multiple of 16 bytes
        byte[] paddedInput = PadStringToMultipleOf16Bytes(input);

        // Split the padded input into 16-byte blocks
        for (int i = 0; i < paddedInput.Length; i += 16)
        {
            byte[] block = new byte[16];
            Array.Copy(paddedInput, i, block, 0, Math.Min(16, paddedInput.Length - i));
            paddedBlocks.Add(block);
        }
    ...
    {
        int keySize = 32; // AES-256 uses a 256-bit key (32 bytes)
        // Console.WriteLine(keySize);
        int blockSize = 16; // AES block size is 128 bits (16 bytes)
        int expandedKeySize = blockSize * (14 + 1); // 15 round keys for AES-256

        roundKeys = new byte[expandedKeySize / 4][];
        ...
        {
            roundKeys[i / 4][i % 4] = key[i];
        }

        for (int i = keySize / 4; i < expandedKeySize / 4; i++)
        {
            byte[] temp = (byte[])roundKeys[i - 1].Clone();
            ...
            for (int j = 0; j < 4; j++)
            {
                roundKeys[i][j] = (byte)(roundKeys[i - (keySize / 4)][j] ^ temp[j]);
            }
        }
    }
    ...
    private byte[,] ParseBlock(byte[] block)
    {
    ...
    private void MixColumns(byte[,] state)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            temp[0] = (byte)(GFMul(state[0, i], 2) ^ GFMul(state[1, i], 3) ^ state[2, i] ^ state[3, i]);
            temp[1] = (byte)(state[0, i] ^ GFMul(state[1, i], 2) ^ GFMul(state[2, i], 3) ^ state[3, i]);
            temp[2] = (byte)(state[0, i] ^ state[1, i] ^ GFMul(state[2, i], 2) ^ GFMul(state[3, i], 3));
            temp[3] = (byte)(GFMul(state[0, i], 3) ^ state[1, i] ^ state[2, i] ^ GFMul(state[3, i], 2));
            for (int j = 0; j < 4; j++)
            {
                state[j, i] = temp[j];
            }
    ...
    {
        List<byte[]> blocks = SplitAndPadInput(input);
        List<byte[]> encryptedBlocks = new List<byte[]>();

        foreach (var block in blocks)
        {
            byte[,] state = ParseBlock(block);
        ...
        // Combine all encrypted blocks into a single byte array
        int totalLength = encryptedBlocks.Count * 16;
        byte[] output = new byte[totalLength];
        int offset = 0;
        foreach (var encryptedBlock in encryptedBlocks)
        {
            Array.Copy(encryptedBlock, 0, output, offset, 16);
            offset += 16;
        }
    ...
    {
        // Inverse shift rows
        // Shift first row right by 0 bytes
    ...
    private void InverseMixColumns(byte[,] state)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            temp[0] = (byte)(GFMul(state[0, i], 0x0e) ^ GFMul(state[1, i], 0x0b) ^ GFMul(state[2, i], 0x0d) ^ GFMul(state[3, i], 0x09));
            temp[1] = (byte)(GFMul(state[0, i], 0x09) ^ GFMul(state[1, i], 0x0e) ^ GFMul(state[2, i], 0x0b) ^ GFMul(state[3, i], 0x0d));
            temp[2] = (byte)(GFMul(state[0, i], 0x0d) ^ GFMul(state[1, i], 0x09) ^ GFMul(state[2, i], 0x0e) ^ GFMul(state[3, i], 0x0b));
            temp[3] = (byte)(GFMul(state[0, i], 0x0b) ^ GFMul(state[1, i], 0x0d) ^ GFMul(state[2, i], 0x09) ^ GFMul(state[3, i], 0x0e));
            for (int j = 0; j < 4; j++)
            {
                state[j, i] = temp[j];
            }
    ...
    public List<byte[]> SplitInputIntoBlocks(byte[] data)
    {
        ...
        {
            byte[] block = new byte[16];
            Array.Copy(data, i, block, 0, Math.Min(16, data.Length - i));
            blocks.Add(block);
        }
    ...
    public string Decrypt(byte[] encryptedData)
    {
        ...
        // Combine all decrypted blocks into a single byte array
        int totalLength = decryptedBlocks.Sum(b => b.Length);
        byte[] output = new byte[totalLength];
        int offset = 0;
        foreach (var decryptedBlock in decryptedBlocks)
        {
            Array.Copy(decryptedBlock, 0, output, offset, decryptedBlock.Length);
